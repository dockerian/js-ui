 ...
 *  which demonstrates usage of ES6 async/await instead of
 *  using callbacks. The command name is the filename and the
 *  exported "command" function is the command.
 *
 * Example usage:
 *   browser.openHomeAboutPage();
 *
...
module.exports = {
  command: async function () {
    ...
    const result = await this.elements('css selector', '.main a')
    this.assert.strictEqual(result.value.length, 9)
  }
}
